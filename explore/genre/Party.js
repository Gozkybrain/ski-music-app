...
const Party = () => {
  ...
  const fetchPopSongs = async () => {
    try {
      const response = await axios.get(`http://ws.audioscrobbler.com/2.0/?method=tag.gettoptracks&tag=party&api_key=${LASTFM_API_KEY}&format=json`);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.genreCard}>
        <Image source={require('../../assets/bunny.png')} style={styles.genreImage} />
        <View style={styles.bg}> 
          <Text style={styles.genreCardText}>Party</Text>
        </View>
      </View>
